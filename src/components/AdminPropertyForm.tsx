...
export default function AdminPropertyForm({ initialProperty }: AdminPropertyFormProps) {
  ...
  const [shortDescription, setShortDescription] = useState(
    initialProperty?.shortDescription ?? ""
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl space-y-8">
      {error && (
        <p className="rounded-md bg-terracotta/10 px-4 py-3 text-sm text-terracotta">
          {error}
        </p>
      )}
      ...
      <div className="rounded-lg border border-black/10 bg-white p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <input
            ...
            placeholder="Location (e.g. Midoun, Djerba)"
            className="rounded-md border border-black/15 px-4 py-2.5 text-sm"
          />
          <input
            required
            ...
            placeholder="Price per night (TND)"
            className="rounded-md border border-black/15 px-4 py-2.5 text-sm"
          />
          <select
            value={propertyType}
        ...
        <input
          ...
          placeholder="Short description (used in listings & SEO)"
          className="mt-4 w-full rounded-md border border-black/15 px-4 py-2.5 text-sm"
        />
        <textarea
          required
          value={description}
      ...
      <button
        ...
        className="rounded-md bg-djerba px-6 py-3 text-sm font-medium text-cream hover:bg-djerba-dark disabled:opacity-50"
      >
